"""DLAM 2026 — residual encoder for direct 336-step forecasting (v3).
...
*   The per-unit level swings by +-15-20% from one fortnight to the next, in a
    ...
    - `trend` (the time index) is DROPPED in the final model.

*   The switches below are recorded in the checkpoint stats, so predict.py
    rebuilds the exact feature block that was trained on.
...
H = 336                  # required prediction horizon
HOW = 168                # hours in a week
ROLL = 168               # rolling block length, ending at t - min(lags)
MIN_LAG = 672            # hard floor; anything shorter is hidden at test
...
# ---- feature switches ---------------------------------------------------------
# The defaults are the configuration of the submitted model (final checkpoint):
# `trend` dropped, day-of-month harmonics on, cross-unit means on.  They can be
# overridden through environment variables for the ablations in the report:
#     DROP_FEATURES=""        keep `trend`            (rows "v2" and "A" / "B")
#     USE_DOM=0 USE_GLOBAL=0  no harmonics / no means (rows "v2")
# Whatever was active at training time is recorded in the checkpoint stats, so
# predict.py always rebuilds the feature block the checkpoint was trained on,
# independent of these variables.
DROP_FEATURES = tuple(f for f in os.environ.get("DROP_FEATURES", "trend").split(",") if f)
USE_DOM = os.environ.get("USE_DOM", "1") == "1"          # day-of-month harmonics
GLOBAL_OF = [                    # cross-unit hourly means of these covariates
    "queue_pressure_forecast", "network_pressure_forecast",
    "event_load_forecast", "shock_risk", "service_irregularity_risk_forecast",
    "throughput_disruption_risk_forecast", "unit_reliability_forecast",
    "maintenance_known",
]
USE_GLOBAL = os.environ.get("USE_GLOBAL", "1") == "1"    # cross-unit means
# -----------------------------------------------------------------------------
...
def _how_of(index: pd.DatetimeIndex) -> np.ndarray:
    return (index.dayofweek * 24 + index.hour).to_numpy(np.int64)
...
def target_matrix(train: pd.DataFrame, st: dict):
    ...
    grid = pd.date_range(df[TIME].min(), df[TIME].max(), freq="h")
...
def build_block(cov, st, Y, abs_hours, grid_start, lags=()):
    ...
    stamps = pd.DatetimeIndex(
        [grid_start + pd.Timedelta(hours=int(h)) for h in abs_hours])
    how = _how_of(stamps)
